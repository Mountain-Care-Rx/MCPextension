...
function applyToolbarVisibilityToAllTabs(isVisible) {
  // Only match tabs with contacts or conversations in the path
  browserAPI.tabs.query({ url: [
    "*://app.mtncarerx.com/*contacts*",
    "*://app.mtncarerx.com/*conversations*"
  ] }, (tabs) => {
    if (!tabs || tabs.length === 0) {
    ...
    tabs.forEach(tab => {
      // Double check the URL before sending the message
      if (
        tab.url &&
        tab.url.includes('app.mtncarerx.com') &&
        (tab.url.includes('contacts') || tab.url.includes('conversations'))
      ) {
        browserAPI.tabs.sendMessage(
          tab.id,
